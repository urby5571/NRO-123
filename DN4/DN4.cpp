...
int main() {
    ...
    {
        // Gauss-Seidlova metoda z OpenMP
        for (int i = 0; i < n; i++) { // Iteracija po vrsticah
            double sum = 0.0;

            // Paralelizirana notranja zanka z OpenMP
            #pragma omp parallel for default(shared) private(sum) reduction(+:sum)
            for (int k = 0; k < n; k++) { // Iteracija po stolpcih
                if (k != i) { // Ne upoštevamo diagonale
                    sum += A[i][k] * T[k];
                }
            }

            // Sprotna posodobitev rešitve novega T-ja
            double T_nov = (b[i] - sum) / A[i][i];
            T[i] = T_nov;
        }

    }
/*Metoda je delno paralelizirana.Paralelizacijo lahko izvedemo v notranji zanki, kjer se izračunava vsota
sigma. To je mogoče zato, ker so posamezni členi v vsoti (A[i][j]⋅T[j]) med seboj popolnoma neodvisni.
Zaradi te neodvisnosti je uporaba OpenMP za paralelno izvajanje varna in učinkovita.
Zunanja zanka pa ostaja sekvenčna, saj Gauss - Seidelova metoda zahteva sprotno posodabljanje vrednosti T[i].
Pri izračunu nove vrednosti T[i] se uporabljajo že posodobljene vrednosti T[j], kjer je j < i.
Zaradi te odvisnosti od rezultatov prejšnjih iteracij popolna paralelizacija metode ni možna.*/
